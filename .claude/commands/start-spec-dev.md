...
**Important for PaperAlchemy**:
- Reference code in `_reference/src/` for patterns — but write from scratch
- Create notebook: `notebooks/specs/S{x}.{y}_{slug}.ipynb` (backend specs only)
- Ensure spec is testable via UI (Gradio endpoint or Next.js page)
- All answers must enforce citation format (inline [1],[2] + arXiv links)
...
Audit: code existence, tests, lint, tangible outcomes, notebook created, integration wiring.
...
5. Notebook:       {CREATED/SKIPPED} -- notebooks/specs/S{x}.{y}_{slug}.ipynb
6. Spec Summary:   {APPENDED/SKIPPED} -- docs/spec-summaries.md
...
6. For reference patterns, look in `_reference/src/` (old code moved there for study)
